...
// userid  1 == verfiy
    ...
    public static void updateBanner(RequestBody parameters,
                                     final Context context,
                                     final CommonInterfaces.updateBanner delegate) {
        Call<ResponseBody> responseBodyCall = ApiClient.getInstance().getApiService().updateBanner(parameters);
        responseBodyCall.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                try {
                    ...
                    try {
                        ...
                        if (jsonObject.getString("rccode").equalsIgnoreCase("200")) {
                            //   JSonParser.userDetailParse(res);
                            delegate.updateBannerSuccess(jsonObject.getString("message"));
                        } else {
                            delegate.updateFailure(jsonObject.getString("message"));
                            //  PopUpWindow.dError(context, "Failed " + jsonObject.getString("message"));
                        }
                    } catch (JSONException e) {
                        e.printStackTrace();
                    ...
                    e.printStackTrace();
                    delegate.updateFailure("Login failed");
                }
            }
            ...
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                try {
                    delegate.updateFailure(call.execute().message().toString());
                } catch (Exception e) {
                    e.printStackTrace();
                    delegate.updateFailure("Login failed. Try again");
                }
            }
        });
    }
